...
pub fn run_simulations(cfg: &SimulationsCLIConfig) {
    run_simulations_handle_errors(&cfg.output_cfg, &cfg.sim_cfg);
}

fn run_simulations_handle_errors(output_cfg: &OuputConfig, sim_cfg: &SimConfig) {
...
fn run_simulations_inner(output_cfg: &OuputConfig, sim_cfg: &SimConfig) -> Result<(), Box<dyn Error>> {
...
pub fn reproduce_simulations(cfg: &ReproduceConfig) {
    match extract_sim_config(&cfg.input_path) {
        ...
        Ok(sim_cfg) => {
            ...
            run_simulations_handle_errors(&cfg.output_cfg, &sim_cfg);
        }
    }
}
